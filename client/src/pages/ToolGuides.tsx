...
function ToolLesson({ id }: { id: ToolGuide["id"] }) {
  ...
  return <div className="kit-page tools-page tool-lesson-page"><KitHeader active={guide.lesson} /><main className="tools-main"><LessonCopyGuide data={copyGuide}/>{guide.id === "firebase" && <video src="/images/HOME LIVE B.mp4" className="firebase-living-room" autoPlay muted loop playsInline controls />}{guide.intro}<section className="tool-guide standalone-tool-guide" id={guide.id}><div className="tool-title tool-title-big"><span>0{guide.lesson}</span><div><h2>認識 {guide.tool}：</h2>{guide.id === "firebase" && <p>先記住這張安全地圖，第 5 課才開始按。</p>}</div></div>{guide.toolIntro && <div className="tool-intro-card"><div><b>{guide.toolIntro.name} 是甚麼</b><p>{guide.toolIntro.what}</p></div><div className="tool-intro-trick"><b>☆ 絕招</b><p>{guide.toolIntro.trick}</p></div></div>}<div className="tool-guide-hero"><ToolMascot src={guide.mascot.src} alt={guide.mascot.alt} /></div><div className="tool-image">{guide.view}</div>{guide.overview}{guide.id !== "firebase" && <div className="task-unit-list">{guide.steps.map((step, index) => <TaskUnit key={step.title} step={step} number={index + 1} />)}</div>}<div className="tool-result"><CheckCircle2 /><div><b>本課完成</b><p>{guide.completion || guide.article.standard}</p></div><ToolMascot src={(guide.finishArt || guide.mascot).src} alt={(guide.finishArt || guide.mascot).alt} /></div></section><LessonPager current={guide.lesson} /></main><KitFooter /></div>;
}
