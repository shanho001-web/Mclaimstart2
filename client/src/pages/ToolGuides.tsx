...
export function FirebaseSecurityPrimer() {
  return <section className="firebase-opening" aria-labelledby="firebase-welcome-title"><div className="firebase-welcome-copy"><p><Lock size={16}/>{firebaseWelcomeMessage.eyebrow}</p><h1 id="firebase-welcome-title">{firebaseWelcomeMessage.title}<em>{firebaseWelcomeMessage.emphasis}</em></h1><span>請朋友到家吃飯，客廳可以讓大家自在進出；但收據、管理員帳戶和資料權限，就像重要物品，應該鎖進房間。</span></div><aside className="firebase-danger-warning" aria-label="不設防的危險警告"><p className="firebase-danger-label">⚠ 先看危險：不設定後端的後果</p><ol><li><b>任何人都能讀取所有資料</b>：Firebase 預設測試模式開放讀寫，知道網址的人就能拉走整份資料。</li><li><b>任何人都能竄改或刪除資料</b>：不只偷看，還能改金額、刪記錄、上載假收據。</li><li><b>管理員帳戶形同虛設</b>：登入畫面可以造假，沒有伺服器規則把關，會員自稱管理員也攔不住。</li></ol><p className="firebase-danger-why">所以這課不是「加分題」：Firebase 的 Authentication、Firestore Rules、Storage Rules 三道防線，是讓「只有你」能碰資料的必要設定，不是可選的美化。</p></aside><section className="firebase-security-brief" aria-labelledby="firebase-security-title">
...
function ToolLesson({ id }: { id: ToolGuide["id"] }) {
  ...
  return <div className="kit-page tools-page tool-lesson-page"><KitHeader active={guide.lesson} /><main className="tools-main"><LessonCopyGuide data={copyGuide}/>{guide.id === "firebase" && <img src="/images/firebase-welcome-living-room.png" className="firebase-living-room" alt="客廳歡迎朋友、重要物品鎖進房間的插圖"/>}{guide.intro}<section className="tool-guide standalone-tool-guide" id={guide.id}><div className="tool-title tool-title-big"><span>0{guide.lesson}</span><div><h2>認識 {guide.tool}{guide.id === "firebase" ? "：" : ""}</h2>{guide.id === "firebase" && <p>先記住這張安全地圖，第 5 課才開始按。</p>}</div></div>{guide.toolIntro && <div className="tool-intro-card"><div><b>{guide.toolIntro.name} 是甚麼</b><p>{guide.toolIntro.what}</p></div><div className="tool-intro-trick"><b>☆ 絕招</b><p>{guide.toolIntro.trick}</p></div></div>}<div className="tool-guide-hero"><ToolMascot src={guide.mascot.src} alt={guide.mascot.alt} /></div><div className="tool-image">{guide.view}</div>{guide.overview}{guide.id !== "firebase" && <div className="task-unit-list">{guide.steps.map((step, index) => <TaskUnit key={step.title} step={step} number={index + 1} />)}</div>}<div className="tool-result"><CheckCircle2 /><div><b>本課完成</b><p>{guide.completion || guide.article.standard}</p></div><ToolMascot src={(guide.finishArt || guide.mascot).src} alt={(guide.finishArt || guide.mascot).alt} /></div></section><LessonPager current={guide.lesson} /></main><KitFooter /></div>;
}
