...
export default function ClaimKit() {
  ...
  return <div className="kit-page"><KitHeader active={5}/><main className="kit-main">
    ...
    <section className="kit-section" data-part="01" id="website-code"><div className="section-tab">{claimAssemblyParts.website.title}</div><h2>第一部分：在 VS Code 換三個網頁檔案。</h2><p className="section-lead">先按上方的檔案名稱選擇 <b>index.html</b>、<b>dashboard.html</b> 或 <b>vercel.json</b>。然後只做同一個動作：在 VS Code 左邊 Explorer 開同名檔案，複製下方 code，回檔案全選、貼上、儲存。index.html 和 dashboard.html 貼好後，要把 code 內 6 個 firebaseConfig 的 <b>YOUR_</b> 欄位換成你在 PART 00 複製的真實值，並把 <b>YOUR_ADMIN_EMAIL</b> 改成你打算使用的管理員 Email；vercel.json 不用改任何欄位。</p><FirebaseConfigBreakdown/><div className="code-tab-row">{websiteSamples.map(item=><button className={item.id === websiteActive ? "code-tab active" : "code-tab"} onClick={()=>setWebsiteActive(item.id)} key={item.id}>{item.label}</button>)}</div><div className="sample-explain"><Code2 size={18}/><div><b>{websiteSample.file}</b><span>{websiteSample.note}</span></div></div><CodeBlock code={claimCode[websiteActive]} fileName={websiteSample.file}/><HumanSteps steps={[`VS Code 左邊 Explorer → 開 ${websiteSample.file}`, "回這張卡按「複製 code」", "回同一個檔案按 Ctrl/Cmd + A → 貼上 → Ctrl/Cmd + S 儲存", ...(websiteActive === "vercelJson" ? ["vercel.json 不需要改欄位；儲存即可"] : ["對照上方逐行卡，把 code 內 6 個 firebaseConfig 的 YOUR_ 欄位換成 PART 00 複製的真實值，並把 YOUR_ADMIN_EMAIL 改成自己的管理員 Email"])]}/></section>
