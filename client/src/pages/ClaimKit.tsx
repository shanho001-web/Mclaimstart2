...
export const claimLessonArticle = {
  heading: "把 Claim 控制台接上 Firebase，",
  ease: "只分兩部分完成。",
  summary: "前四課已完成網站、版本、網址和安全概念。現在不用再理解新理論：第一部分在 VS Code 放入兩個完整網頁檔案；第二部分回 Firebase 建立自己的管理員帳戶，再貼兩份 Rules。",
  flow: ["替換 index.html", "替換 dashboard.html", "建立唯一管理員", "發布兩份 Rules"],
  finish: "登入後看見守護員管理主頁，而且可以查看 Claim、開收據和更新狀態，便代表兩部分已正確集合。",
};
...
export default function ClaimKit() {
  ...
  return <div className="kit-page"><KitHeader active={5}/><main className="kit-main">
    ...
    <section className="kit-hero claim-hero-copy claim-article-lead"><div><p className="kit-kicker">LESSON 05 / 05 · 管理員專用 Claim 控制台</p><p className="lesson-article-promise">最後一課 · 把前面準備好的設定集合起來</p><h1>{claimLessonArticle.heading}<em>{claimLessonArticle.ease}</em></h1><p className="lesson-article-summary">{claimLessonArticle.summary}</p><ol className="lesson-article-flow">{claimLessonArticle.flow.map((item, index) => <li key={item}><b>{index + 1}</b>{item}</li>)}</ol><div className="kit-pills"><span><Code2/> VS Code</span><span><ShieldCheck/> Firebase Rules</span><span><Globe2/> Vercel</span></div><a className="tool-guide-launch" href="/lesson-4-firebase"><MonitorPlay size={17}/> 忘了安全分工？回第 4 課</a></div><div className="kit-diagram"><div className="diagram-node"><Code2/>兩個網頁</div><i/><div className="diagram-node"><ShieldCheck/>兩份 Rules</div><i/><div className="diagram-node strong"><Globe2/>Vercel 測試</div></div></section>
    ...
    <section className="kit-section secure-finish"><div><p className="section-tab">本課完成</p><h2>回 Vercel 網址，用管理員 Email 登入。</h2><p>{claimLessonArticle.finish}</p></div><ShieldCheck className="finish-shield"/></section><LessonPager current={5}/>
  </main><KitFooter/></div>;
}
