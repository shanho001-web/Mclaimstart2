...
export default function ClaimKit() {
  ...
  return <div className="kit-page"><KitHeader active={5}/><main className="kit-main">
    ...
    <section className="real-files-strip"><img className="mascot-inline mascot-folder" src={folderMascotUrl} alt="小守護員帶著資料夾"/><div><b>第五課要交給 Vercel 的 3 個實戰檔案</b><p>先建立並放入同一個 GitHub repository；然後接上 Firebase，最後 Vercel Deploy。</p></div><ol><li><span>01</span><b>index.html</b><small>會員登入／忘記密碼</small></li><li><span>02</span><b>dashboard.html</b><small>提交、查看與管理 claim</small></li><li><span>03</span><b>vercel.json</b><small>/ 及 /dashboard 路由</small></li></ol></section>
    ...
    <section className="kit-section" data-part="01" id="code"><div className="section-tab">PART 01 · 重寫網站 code</div><h2>先將四個檔案放到同一個 GitHub repository。</h2><p className="section-lead">每次只處理一個檔案：選檔案、複製完整 code、回同名檔案貼上。</p><div className="code-tab-row">{samples.map(item=><button className={item.id === active ? "code-tab active" : "code-tab"} onClick={()=>setActive(item.id)} key={item.id}>{item.label}</button>)}</div><div className="sample-explain"><Code2 size={18}/><div><b>{sample.file}</b><span>{sample.note}</span></div></div><CodeBlock code={claimCode[active]} fileName={sample.file}/><HumanSteps steps={[`確認上方目前選的是 ${sample.file}`, "按「複製 code」一次", `回同名 ${sample.file}，先全選舊內容，再貼上完整 code`]} tip="完成一個檔案才按下一個標籤；不要把不同檔案的 code 混在一起。"/></section>
    <section className="kit-section split-kit" data-part="02" id="firebase"><div><div className="section-tab">PART 02 · Firebase 後台</div><h2>按這個順序開啟四道門。</h2><div className="console-steps"><article><b>01</b><div><h3>開啟 Authentication</h3><p className="step-where">Firebase Console → Build → Authentication。</p><HumanSteps steps={["按 Get started", "在 Sign-in method 開啟 Email/Password", "到 Users 建立第一個熟人帳戶"]} tip="先只開 Email/Password；其他登入方式暫時不用開。"/></div></article><article><b>02</b><div><h3>建立 Web app</h3><p className="step-where">Project settings → Your apps → Web。</p><HumanSteps steps={["註冊 Web app", "複製 firebaseConfig", "貼到 index.html 和 dashboard.html 的 YOUR_... 位置"]} tip="同一組 firebaseConfig 要貼進兩個檔案。"/></div></article><article><b>03</b><div><h3>開啟 Firestore</h3><p className="step-where">Build → Firestore Database → Create database。</p><HumanSteps steps={["建立資料庫", "打開 Rules", "貼上「報帳 Rules」卡；改好管理員 email 後才按 Publish"]} tip="Publish 前先檢查管理員 email 是否正確。"/></div></article><article><b>04</b><div><h3>開啟 Storage</h3><p className="step-where">Build → Storage → Get started。</p><HumanSteps steps={["建立 Storage", "打開 Rules", "貼上「收據 Rules」卡，再按 Publish"]} tip="不要保留測試模式的公開規則。"/></div></article></div></div><aside className="test-panel"><img className="mascot-inline mascot-security" src={securityMascotUrl} alt="小守護員指向安全盾牌"/><CloudCog size={24}/><h3>三個帳戶測試</h3><p>每次發布 Rules 後都要做。這不是形式，而是確認資料隔離真的生效。</p><ol><li>會員 A 提交一張收據和 claim。</li><li>會員 B 登入後，嘗試讀取 A 的 claim／收據：應被拒絕。</li><li>管理員登入後，檢查可讀取並更新 claim 狀態。</li></ol><small>Firestore Rules 模擬器可在 Firestore → Rules 開啟，先測試再 Publish。[1]</small></aside></section>
