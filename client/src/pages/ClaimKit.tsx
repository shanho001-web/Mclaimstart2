...
const samples: { id: Sample; label: string; file: string; note: string; tone?: "firestore" | "storage" }[] = [
  ...
  { id:"firestoreRules", label:"管理員 Rules", file:"Firestore → Rules", note:"只有指定管理員 Email 可讀取、建立、更新或刪除 Claim。", tone:"firestore" },
  { id:"storageRules", label:"收據 Rules", file:"Storage → Rules", note:"只有指定管理員 Email 可讀取或寫入 claims 收據。", tone:"storage" },
...
export default function ClaimKit() {
  ...
  return <div className="kit-page"><KitHeader active={5}/><main className="kit-main">
    ...
    <section className="kit-section" data-part="01" id="website-code"><div className="section-tab">{claimAssemblyParts.website.title}</div><h2>第一部分：在 VS Code 放好三個網頁檔案。</h2><p className="section-lead">先按上方檔案名稱選 <b>index.html</b>、<b>dashboard.html</b> 或 <b>vercel.json</b>。三個檔案做法不同：<b>index.html</b> 在 VS Code 開原有檔案「覆蓋」；<b>dashboard.html</b> 和 <b>vercel.json</b> 是第 1 課沒有的新檔案，要在 VS Code 左邊 Explorer 按右鍵 → New File「新增」。複製下方 code 貼好後，index.html 和 dashboard.html 要把 6 個 firebaseConfig 的 <b>YOUR_</b> 換成 PART 00 複製的真實值，並把 <b>YOUR_ADMIN_EMAIL</b> 改成管理員 Email；vercel.json 不用改欄位。</p><FirebaseConfigBreakdown/><div className="code-tab-row">{websiteSamples.map(item=><button className={item.id === websiteActive ? "code-tab active" : "code-tab"} onClick={()=>setWebsiteActive(item.id)} key={item.id}>{item.label}</button>)}</div><div className="sample-explain"><Code2 size={18}/><div><b>{websiteSample.file}</b><span>{websiteSample.note}</span></div></div><CodeBlock code={claimCode[websiteActive]} fileName={websiteSample.file}/><HumanSteps steps={websiteActive === "indexHtml" ? ["VS Code 左邊 Explorer → 開原有 index.html", "回這張卡按「複製 code」", "回 index.html 按 Ctrl/Cmd + A 全選 → 貼上 → Ctrl/Cmd + S 儲存（覆蓋）", "把 6 個 firebaseConfig 的 YOUR_ 換成真實值，並把 YOUR_ADMIN_EMAIL 改成管理員 Email"] : [`VS Code 左邊 Explorer 空白處按右鍵 → New File`, `輸入 ${websiteActive === "dashboardHtml" ? "dashboard.html" : "vercel.json"} 按 Enter（新增）`, "回這張卡按「複製 code」→ 回新檔案貼上 → Ctrl/Cmd + S 儲存", ...(websiteActive === "vercelJson" ? ["vercel.json 不需要改欄位；儲存即可"] : ["把 6 個 firebaseConfig 的 YOUR_ 換成真實值，並把 YOUR_ADMIN_EMAIL 改成管理員 Email"])]}/></section>
