...
export const steps: CourseStep[] = [
  { id:"vscode", number:"01", title:"打開 VS Code 工作枱", short:"下載 → Folder → Terminal", tool:"VS Code", reward:"", goal:"", guides:[
    { visual:"download", title:"下載並打開 VS Code", instruction:"在這張卡下方的「下載 VS Code」按鈕，開啟官方下載頁。", detail:"下載並安裝 VS Code。", microSteps:["按下方「下載 VS Code」", "選自己的 Mac 或 Windows 版本", "Mac：雙擊下載的 .dmg 檔，把 Visual Studio Code.app 拖進 Applications，再從 Applications 雙擊開啟；Windows：雙擊安裝檔，跟畫面完成安裝"], result:"你會看到藍色 VS Code 圖示和開始畫面。", link:{ label:"下載 VS Code", href:"https://code.visualstudio.com/Download" }, tip:"不要從不明網站下載。這個按鈕會直接前往 VS Code 官方下載頁。" },
    ...
    { visual:"terminal", title:"用 Terminal 建立三個網站檔案", instruction:"VS Code 最上方選單 → Terminal → New Terminal。", detail:"貼上建立檔案的指令。", microSteps:["選自己的 Mac 或 Windows code 卡，按「複製 code」", "回黑色 Terminal 貼上完整 code，再按 Enter"], result:"左側 Explorer 出現 index.html、style.css、script.js 三個檔案；Terminal 沒有紅色 error。", code:"macFiles", choices:["macFiles", "windowsFiles"], tip:"黑色 Terminal 是本關唯一要貼指令的位置；只需貼一次。Windows 若出現紅色「無法辨識 ni」或 command not found，代表 Terminal 是 Command Prompt：按 Terminal 右邊的下箭頭選 PowerShell，再貼一次。" }
  ]},
  { id:"welcome", number:"02", title:"讓小可愛在主頁歡迎你", short:"HTML → CSS → 動畫", tool:"VS Code 左側 Explorer", reward:"", goal:"", guides:[
    ...
    { visual:"copy", title:"把 CSS code 放進 style.css", instruction:"VS Code 左側 Explorer → style.css。", detail:"完整覆蓋 style.css 的內容。", microSteps:["點 style.css，再按 Ctrl/Cmd + A 全選", "按下方「複製 code」", "貼上完整內容，再按 Ctrl/Cmd + S 儲存"], result:"style.css 內會看到 .stage、.guardian 和 @keyframes 這些文字。", code:"styleCss", tip:"一定是 style.css，不是 styles.css；少一個或多一個 s，主頁就找不到顏色和動畫。" },
    { visual:"copy", title:"把 JavaScript code 放進 script.js", instruction:"VS Code 左側 Explorer → script.js。", detail:"完整覆蓋 script.js 的內容。", microSteps:["點 script.js，再按 Ctrl/Cmd + A 全選", "按下方「複製 code」", "貼上完整內容，再按 Ctrl/Cmd + S 儲存"], result:"script.js 會看到 addEventListener 和「第一個歡迎頁完成」文字。", code:"scriptJs", tip:"三個檔案都儲存後才測試。看到檔案名稱旁沒有白點或星號，代表已儲存。" },
    { visual:"launch", title:"在瀏覽器打開第一個網站", instruction:"VS Code 左側 Explorer → 在 index.html 上按右鍵。", detail:"從資料夾雙擊 index.html。", microSteps:["在 VS Code 左側 Explorer 的 index.html 上按右鍵", "選 Reveal in Finder／Show in File Explorer", "彈出資料夾後，雙擊 index.html"], result:"瀏覽器會打開小守護員主頁；按黃色按鈕後，文字變成「第一個歡迎頁完成」。", tip:"看不到浮動動畫時，回 VS Code 檢查 style.css 和 script.js 是否已儲存。" }
  ]},
];
