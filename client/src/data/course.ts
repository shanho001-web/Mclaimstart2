...
export const steps: CourseStep[] = [
  { id:"vscode", number:"01", title:"打開 VS Code 工作枱", short:"下載 → Folder → Terminal", tool:"VS Code", reward:"", goal:"", guides:[
    ...
    { visual:"terminal", title:"用 Terminal 建立三個網站檔案", instruction:"VS Code 最上方選單 → Terminal → New Terminal。", detail:"貼上建立檔案的指令。", microSteps:["點 VS Code 上方 Terminal → New Terminal，下方會出現黑色 Terminal", "選自己的 Mac 或 Windows code 卡，按「複製 code」", "回黑色 Terminal 貼上完整 code，再按 Enter"], result:"左側 Explorer 出現 index.html、style.css、script.js 三個檔案；Terminal 沒有紅色 error。", code:"macFiles", choices:["macFiles", "windowsFiles"], tip:"黑色 Terminal 是本關唯一要貼指令的位置；只需貼一次。Windows 若出現紅色「無法辨識 ni」或 command not found，代表 Terminal 是 Command Prompt：按 Terminal 右邊的下箭頭選 PowerShell，再貼一次。" }
  ]},
