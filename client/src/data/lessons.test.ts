...
describe("five-lesson course navigation", () => {
  ...
  it("places the three tool lessons between publishing and Claim", () => {
    ...
    expect(getLesson(3).short).toBe("Vercel");
    expect(getLesson(4).short).toBe("Firebase");
  ...
  it("resolves every course-navigation link and the full previous-next learning path", () => {
    ...
    expect(getCourseNavHref(3)).toBe("/lesson-3-vercel");
    expect(getCourseNavHref(4)).toBe("/lesson-4-firebase");
    expect(getCourseNavHref(5)).toBe("/claim-kit");

    expect(getLessonPager(1)).toMatchObject({ next: { href: "/lesson-2-github" } });
    expect(getLessonPager(2)).toMatchObject({ previous: { href: "/" }, next: { href: "/lesson-3-vercel" } });
    expect(getLessonPager(3)).toMatchObject({ previous: { href: "/lesson-2-github" }, next: { href: "/lesson-4-firebase" } });
    expect(getLessonPager(4)).toMatchObject({ previous: { href: "/lesson-3-vercel" }, next: { href: "/claim-kit" } });
    expect(getLessonPager(5)).toMatchObject({ previous: { href: "/lesson-4-firebase" }, next: undefined });
  });
});
